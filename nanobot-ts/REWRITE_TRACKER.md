...
| Channels | 80% | Strong foundation | Generic `BaseChannel`, `MessageBus`, `ChannelManager`, Telegram implementation, `channels message`, no echo | More channel types, inbound routing into real agent/runtime, richer status/reporting | Wire inbound bus to real runtime |
| Agent | 10% | Stub | Minimal in-memory loop and session map only | No real model/provider execution, no tool loop, no memory/session behavior parity | Replace stub loop with real runtime contract |
| Session | 5% | Stub | Only implicit session tracking inside `agent/loop.ts` | No dedicated session abstraction, persistence, expiration, metadata, or channel/session mapping layer | Split session handling into its own module |
| Providers | 5% | Stub | CLI placeholder for provider login only | No provider abstraction, auth flow, model config, token handling, or runtime integration | Define provider interface and first real provider path |
