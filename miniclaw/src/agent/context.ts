...
function buildIdentitySection(workspacePath: string): string {
  ...
  const parts = [
    ...
    "## Task Policy",
    "",
    "- Review TASKS.md active jobs on every turn before deciding how to respond.",
    "- Keep unfinished jobs progressing or updated when relevant to the user's request.",
    "- Treat the task ledger as internal state unless the user explicitly asks for it or uses `/tasks_list`.",
    "",
    ...buildPlatformPolicy(),
    "",
    "## Search & Discovery",
